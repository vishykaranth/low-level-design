 ...
 * strings together into a loop, where for each leetcode.string you could choose to reverse it or not. Among
 * all the possible loops, you need to find the lexicographically biggest leetcode.string after cutting the
 * loop, which will make the looped leetcode.string into a regular one.
 *
 * <p>Specifically, to find the lexicographically biggest leetcode.string, you need to experience two phases:
 *
 ...
 * make the looped leetcode.string into a regular one starting from the character at the cutpoint. And your
 * job is to find the lexicographically biggest one among all the possible regular strings.
 *
 * <p>Example: Input: "abc", "xyz" Output: "zyxcba" Explanation: You can get the looped leetcode.string
 * "-abcxyz-", "-abczyx-", "-cbaxyz-", "-cbazyx-", where '-' represents the looped status. The
 * answer leetcode.string came from the fourth looped one, where you could cut from the middle character 'a'
