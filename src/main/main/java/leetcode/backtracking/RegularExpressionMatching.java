 ...
 * none, otherwise do a simple one to one match. If the end of leetcode.string is reached simultaneously in
 * both leetcode.string and pattern then return true
 */
public class RegularExpressionMatching {
    ...
    private boolean backTrack(int si, int pi, String s, String p) {
        if (si >= s.length() && pi >= p.length())
            return true; // end of the leetcode.string has been reached hence return true
        else {
            if (pi >= p.length()) return false; // pattern has exhausted hence return false
                ...
                if (pi + 1 < p.length() && p.charAt(pi + 1) == '*') {
                    ...
                    return false; // leetcode.string has exhausted and pattern does not contain wildcard hence return
                // false
