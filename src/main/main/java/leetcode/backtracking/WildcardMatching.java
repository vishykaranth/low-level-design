 ...
 * sequence).
 *
 * <p>The matching should cover the entire input leetcode.string (not partial).
 *
 ...
 * "c*a*b") → false
 *
 * <p>Solution: Maintain two indexes one for leetcode.string and other one for the pattern. 1. If the
 * characters match in both the indexes or if the char at pattern is '?' then increment both the
 * indexes. 2. If a star(*) is encountered save the position of star in the given leetcode.string as
 ...
 * leetcode.string and start the new pattern index from starIdx + 1
 */
        ...
        for (int i = 0, l = s.length(); i < l; ) {
            if (j < p.length()) {
                if (s.charAt(i) == p.charAt(j) || p.charAt(j) == '?') {
                    ...
                    starIdx = j;
                    starPosAtStr = i;
                    j++; // increment only pattern index. This is because '*' can match also empty leetcode.string.
                } else if (starIdx != -1) {
                    i = ++starPosAtStr;
                    j = starIdx + 1;
                } else return false;
